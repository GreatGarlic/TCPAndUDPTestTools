...
{
    public partial class UDP_IPv6_Server_Page : Form
    {
        public UDP_IPv6_Server_Page()
        {
            InitializeComponent();
        }
        
        Socket SocketListener = new Socket(AddressFamily.InterNetworkV6, SocketType.Dgram, ProtocolType.Udp);  
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                SocketListener.Close();
                SocketListener = new Socket(AddressFamily.InterNetworkV6, SocketType.Dgram, ProtocolType.Udp);
                Thread thread = new Thread(IPv6ServerMessage);
                    ...
                    case "监听":
                        {
                            if (!SocketListener.IsBound)
                            {
                                ...
                                {
                                    SocketListener.Bind(new IPEndPoint(IPAddress.IPv6Any, Convert.ToInt32(textBox2.Text)));
                                }
        ...
        void IPv6ServerMessage()
        {
            ...
            try
            {
                while (true)
                {
                    EndPoint Remote = (EndPoint)(new IPEndPoint(IPAddress.IPv6Any, 0));
